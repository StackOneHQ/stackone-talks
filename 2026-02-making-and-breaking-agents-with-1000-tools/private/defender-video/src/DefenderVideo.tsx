    ...
    <AbsoluteFill style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", padding: "64px 70px 72px" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", opacity: Math.max(0, mcpIn), transform: `scale(${interpolate(mcpIn, [0, 1], [0.97, 1], { extrapolateLeft: "clamp" })}) translateY(${interpolate(mcpIn, [0, 1], [12, 0], { extrapolateLeft: "clamp" })}px)` }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 18, background: "rgba(0,25,14,0.75)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", border: `2px solid rgba(0,175,102,0.40)`, boxShadow: `0 8px 32px -8px rgba(0,175,102,0.30)`, borderRadius: 22, padding: "22px 36px" }}>
          ...
          <div>
            <div style={{ fontSize: 24, fontWeight: 700, fontFamily: FONTS.sans, color: DARK.textHeading, letterSpacing: "-0.01em" }}>All StackOne MCP actions protected by default</div>
            <div style={{ fontSize: 16, color: DARK.textBody, fontFamily: FONTS.sans, marginTop: 5 }}>Zero-config — defender runs on every tool call, automatically</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", gap: 28, width: "100%", alignItems: "center", opacity: Math.max(0, bottomIn), transform: `perspective(900px) rotateX(${interpolate(bottomIn, [0, 1], [6, 0], { extrapolateLeft: "clamp" })}deg) translateY(${interpolate(bottomIn, [0, 1], [14, 0], { extrapolateLeft: "clamp" })}px)` }}>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, background: "rgba(0,18,10,0.68)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", border: `1.5px solid rgba(0,175,102,0.20)`, borderRadius: 16, padding: "16px 22px", boxShadow: `0 4px 14px -4px rgba(0,0,0,0.06)` }}>
            <StackOneLogo size={24} />
            <div style={{ fontSize: 17, fontFamily: FONTS.mono, color: DARK.textHeading, fontWeight: 600 }}>stackone.com/platform/prompt-injection-guard</div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 14, background: "rgba(0,18,10,0.68)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", border: `1.5px solid rgba(0,175,102,0.20)`, borderRadius: 16, padding: "16px 22px", boxShadow: `0 4px 14px -4px rgba(0,0,0,0.06)` }}>
            <ProviderIcon provider="github" size={24} />
            <div style={{ fontSize: 17, fontFamily: FONTS.mono, color: DARK.textHeading, fontWeight: 600 }}>github.com/stackoneHQ/defender</div>
            <span style={{ marginLeft: "auto", fontSize: 11, fontWeight: 700, fontFamily: FONTS.sans, color: COLORS.primary, background: `${COLORS.primary}18`, border: `1px solid ${COLORS.primary}40`, borderRadius: 6, padding: "3px 9px", letterSpacing: "0.04em" }}>Open source</span>
          </div>
        </div>
