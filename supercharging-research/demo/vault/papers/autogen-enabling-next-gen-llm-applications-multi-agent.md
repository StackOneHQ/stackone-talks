...
source: http://arxiv.org/abs/2308.08155v2
abstract: "AutoGen is an open-source framework that allows developers to build LLM applications via multiple agents that can converse with each other to accomplish tasks. AutoGen agents are customizable, conversable, and can operate in various modes that employ combinations of LLMs, human inputs, and tools."
---
...
- Builds on single-agent foundations like [[react-synergizing-reasoning-and-acting]] and extends them to multi-agent settings
- The multi-agent conversation paradigm complements the single-agent self-reflection in [[reflexion-language-agents-verbal-reinforcement]]
- Tool-use within agents connects to [[toolformer-language-models-teach-themselves-tools]] and [[toolkengpt-augmenting-frozen-language-models-massive-tools]]
- The agent memory and state management parallels [[generative-agents-interactive-simulacra-human-behavior]] at a more practical engineering level
- [[lats-language-agent-tree-search]] could be used within an AutoGen agent to improve individual agent decision-making
